...
function Warren2D(n::Integer,
    ...
    for i = 1:n-1
        ...
        node = TrussNode([xposition, y, 0.0], :free)
        node.id = shortid
...
function Warren2D(xpositions::Vector{<:Real},
    ...
    for i = 1:length(ypositions2)
        ...
        node = TrussNode([xposition, ypositions2[i], 0.0], :free)
        node.id = shortid
    ...
    #dummy load
    loads = [NodeForce(n, [0.0, -1.0, 0.0]) for n in nodes[longid]]
