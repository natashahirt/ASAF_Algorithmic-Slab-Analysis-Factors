...
"""
    BakerTruss(nbays, dx, dy, section; load = [0.0, -10.0, 0.0])
...
- `load::Vector{<:Real}` load (N) applied to free bottom nodes
"""
function BakerTruss(nbays::Integer, dx::Real, dy::Real, section::Asap.AbstractSection, load = [0.0, -10.0, 0.0])
    ...
    for i = 1:n
        ...
        node = TrussNode([xposition, 0.0, 0.0], :free)
        if i == 1
    ...
    for i = 1:n
        ...
        node = TrussNode([xposition, dy, 0.0], :free, :top)
    ...
    for i = 1:nbays/2
        ...
        node = TrussNode([xposition, dy/2, 0.0], :free, :web)
